...
public class Config {
    ...
    //用于存放菜单项的边界：x1,y1,x2,y2,itemid
    java.util.List<int[]> menuItemBounds = new ArrayList<int[]>();
    final static int TITLE_H = 17;
        ...
        switch (key) {
            ...
            default:
                ...
                if (settings.get("phone").equals("Siemens")) {
                    switch (key) {
                        case Mobile.SIEMENS_UP:
                            itemid--;
                            break;
                        case Mobile.SIEMENS_DOWN:
                            itemid++;
                            break;
                        case Mobile.SIEMENS_SOFT1:
                            menuid = 0;
                            break;
                    }
                }
                if (settings.get("phone").equals("Motorola")) {
                    switch (key) {
                        case Mobile.MOTOROLA_UP:
                            itemid--;
                            break;
                        case Mobile.MOTOROLA_DOWN:
                            itemid++;
                            break;
                        case Mobile.MOTOROLA_SOFT1:
                            menuid = 0;
                            break;
                        case Mobile.MOTOROLA_FIRE:
                            doMenuAction();
                            break;
                    }
                }
    ...
    public void mouseReleased(int x, int y) {
        if (y > height - TITLE_H) {
            if (x < width / 2) {
                keyPressed(Mobile.NOKIA_SOFT1);
            } else {
                if (!menuItemBounds.isEmpty()) {
                    int maxId = menuItemBounds.get(menuItemBounds.size() - 1)[4];
                    if (itemid == maxId) {
                        itemid = 0;
                    } else {
                        itemid = maxId + 1;
            ...
            for (int[] item : menuItemBounds) {
                if (x >= item[0] && x < item[2] && y >= item[1] && y < item[3]) {
                    itemid = item[4];
                    doMenuAction();
                    break;
                }
    ...
    public void render() {
		...
		*/
        ...
        if (menuid > 0) {
            gc.setColor(0xFFFFFF);
            gc.drawString("Back", 3, height - TITLE_H, Graphics.LEFT);
        }
        ...
        if (pages >= 1) {
            gc.setColor(0xFFFFFF);
            gc.drawString((page + 1) + "/" + (pages + 1), width - 3, height - TITLE_H, Graphics.RIGHT);
        }
        ...
        for (int i = start; (i < (start + max)) & (i < t.length); i++) {
            ...
            if (menuid == 0 && i > 1 && i < 7) {
                switch (i) {
                    ...
                    case 3:
                        ...
                        break;
                    case 4:
                        label = label + ": " + settings.get("phone");
                        break;
                    case 5:
            ...
            int dx = width / 2;
            int dy = 25 + space + (ah * (i - start));
            if (i == itemid) {
                gc.setColor(0xFFFF00);
                gc.drawString("> " + label + " <", dx, dy, Graphics.HCENTER);
            } else {
                gc.setColor(0xFFFFFF);
                gc.drawString(label, dx, dy, Graphics.HCENTER);
            }
            //append menu item bounds for touch events
            int[] bounds = new int[]{0, dy, width, dy + ah, i};
            menuItemBounds.add(bounds);
        }
